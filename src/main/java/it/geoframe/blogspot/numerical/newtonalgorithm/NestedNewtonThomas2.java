...
public class NestedNewtonThomas2 {
	...
	public NestedNewtonThomas2(int nestedNewton, double newtonTolerance, int MAXITER_NEWT, int VECTOR_LENGTH, List<StateEquation> stateEquation, double delta){
	...
	public void set(double[] x, double[] y, double[] xStar, double[] mainDiagonal, double[] upperDiagonal, double[] lowerDiagonal, double[] rhss,
	...
	public double[] solver(){
		...
		if(nestedNewton == 0) {
			...
			for(int element = 0; element < KMAX; element++) {
				x[element] = Math.min(x[element], xStar[parameterID[element]] );
			}
		}
		...
		for(int i = 0; i < MAXITER_NEWT; i++) {
			...
			for(int j = 0; j < KMAX; j++) {
				if(j==0) {
					fs[j] = stateEquation.get(rheologyID[j]).stateEquation(x[j],y[j],parameterID[j],j) - rhss[j]  + mainDiagonal[j]*x[j] + upperDiagonal[j]*x[j+1];
					dis[j] = stateEquation.get(rheologyID[j]).dStateEquation(x[j],y[j],parameterID[j],j);
					//					System.out.println(j+" "+fs[j]);
				} else if(j==KMAX-1) {
					fs[j] = stateEquation.get(rheologyID[j]).stateEquation(x[j],y[j],parameterID[j],j) - rhss[j] + lowerDiagonal[j]*x[j-1] + mainDiagonal[j]*x[j];
					dis[j] = stateEquation.get(rheologyID[j]).dStateEquation(x[j],y[j],parameterID[j],j);
					//					System.out.println(j+" "+fs[j]);
				} else {
					fs[j] = stateEquation.get(rheologyID[j]).stateEquation(x[j],y[j],parameterID[j],j) - rhss[j] + lowerDiagonal[j]*x[j-1] + mainDiagonal[j]*x[j] + upperDiagonal[j]*x[j+1];
					dis[j] = stateEquation.get(rheologyID[j]).dStateEquation(x[j],y[j],parameterID[j],j);
					//					System.out.println(j+" "+fs[j]);
				}
			...
			if(nestedNewton == 0){
				...
				for(int s = 0; s < KMAX; s++) {
					x[s] = x[s] - dx[s]*delta; //if multiply by delta you get the globally convergent Newton aka Newton damped.
				}
			} else {
				x_outer = x.clone();
				...
				for(int j = 0; j < MAXITER_NEWT; j++) {
					...
					for(int l=0; l < KMAX; l++) {
						if(l==0) {
							fks[l] = stateEquation.get(rheologyID[l]).pIntegral(x[l],y[l],parameterID[l],l) - ( stateEquation.get(rheologyID[l]).qIntegral(x_outer[l],y[l],parameterID[l],l) + stateEquation.get(rheologyID[l]).q(x_outer[l],y[l],parameterID[l],l)*(x[l] - x_outer[l]) ) - this.rhss[l] + mainDiagonal[l]*x[l] + upperDiagonal[l]*x[l+1];
							dis[l] = ( stateEquation.get(rheologyID[l]).p(x[l],y[l],parameterID[l],l) - stateEquation.get(rheologyID[l]).q(x_outer[l],y[l],parameterID[l],l) );
							//							System.out.println(l+" "+fks[l]);
							//							System.out.println(l+" "+dis[l]);
						} else if(l==KMAX-1) {
							fks[l] = stateEquation.get(rheologyID[l]).pIntegral(x[l],y[l],parameterID[l],l) - ( stateEquation.get(rheologyID[l]).qIntegral(x_outer[l], y[l], parameterID[l], l) + stateEquation.get(rheologyID[l]).q(x_outer[l],y[l],parameterID[l],l)*(x[l] - x_outer[l]) ) - this.rhss[l] + lowerDiagonal[l]*x[l-1] + mainDiagonal[l]*x[l];
							dis[l] = ( stateEquation.get(rheologyID[l]).p(x[l],y[l],parameterID[l],l) - stateEquation.get(rheologyID[l]).q(x_outer[l],y[l],parameterID[l],l) );
							//							System.out.println(l+" "+fks[l]);
							//							System.out.println(l+" "+dis[l]);
						} else {
							fks[l] = stateEquation.get(rheologyID[l]).pIntegral(x[l],y[l],parameterID[l],l) - ( stateEquation.get(rheologyID[l]).qIntegral(x_outer[l],y[l],parameterID[l],l) + stateEquation.get(rheologyID[l]).q(x_outer[l],y[l],parameterID[l],l)*(x[l] - x_outer[l]) ) - this.rhss[l]  + lowerDiagonal[l]*x[l-1] + mainDiagonal[l]*x[l] + upperDiagonal[l]*x[l+1];
							dis[l] = ( stateEquation.get(rheologyID[l]).p(x[l],y[l],parameterID[l],l) - stateEquation.get(rheologyID[l]).q(x_outer[l],y[l],parameterID[l],l) );
							//							System.out.println(l+" "+fks[l]);
							//							System.out.println(l+" "+dis[l]);
						}
